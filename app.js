...
store.on("error", () => {
  console.log("ERROR in MONGO SESSTIONS STORE", err);
});
...
app.all("*", (req, res, next) => {
  next(new ExpressError(404, "Hang on! page not found.."));
});
app.use((err, req, res, next) => {
  let {
    statusCode = 500,
    message = "Something really tough but we are on it!",
  } = err;
  res.status(statusCode).render("error.ejs", { message });
  // res.status(statusCode).send(message);
  //  res.send("Hang on!  something went wrong...");
});
app.listen(8080, () => {
